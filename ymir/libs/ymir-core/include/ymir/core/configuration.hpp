...
struct Configuration {
    ...
    struct Video {
        ...
        // Renderer threads are created at Saturn construction time from these defaults.
        // Emscripten (single-threaded wasm build) cannot spawn threads, so default them off there.
#ifdef __EMSCRIPTEN__
        static constexpr bool kDefaultThreadedRendering = false;
#else
        static constexpr bool kDefaultThreadedRendering = true;
#endif

        /// @brief Runs the VDP1 renderer in a dedicated thread.
        util::Observable<bool> threadedVDP1 = kDefaultThreadedRendering;

        /// @brief Runs the VDP2 renderer in a dedicated thread.
        util::Observable<bool> threadedVDP2 = kDefaultThreadedRendering;

        /// @brief Runs the VDP2 deinterlacer in a dedicated thread, if the VDP2 renderer is running in a thread.
        util::Observable<bool> threadedDeinterlacer = kDefaultThreadedRendering;
    } video;
