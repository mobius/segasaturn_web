...
private:
    ...
    // NOTE: uint64 because the full 4 GiB address space does not fit in a 32-bit size_t (e.g. wasm32)
    static constexpr uint64 kAddressSpaceSize = 1ull << 32ull;
    static constexpr size_t kInstructionSize = sizeof(uint16); // breakpoints must be instruction-aligned
    static constexpr uint64 kBreakpointMapSize = kAddressSpaceSize / kInstructionSize / kBitsPerByte;
    static constexpr uint64 kWatchpointMapSize = kAddressSpaceSize; // only 2 bits per byte are used
    ...
    struct ChunkedMemory {
        ...
        // NOTE: kSize uses uint64 because 1 << 32 overflows a 32-bit size_t (e.g. wasm32)
        static constexpr uint64 kSize = 1ull << sizeBits;
        static constexpr size_t kChunkSize = static_cast<size_t>(1) << chunkSizeBits;
        static constexpr size_t kChunkMask = kChunkSize - 1;
        static constexpr size_t kNumChunks = static_cast<size_t>(kSize / kChunkSize);
