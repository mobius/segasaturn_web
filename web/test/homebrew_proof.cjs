#!/usr/bin/env node
// Homebrew proof: load real BIOS + original homebrew disc, assert non-black
...
const DEFAULT_DISC = path.join(__dirname, "homebrew", "ymir_homebrew_color.iso");
const DEFAULT_OUT = path.join(ROOT, "build-web", "homebrew-proof.png");
const FRAMES = Number(process.env.YMIR_FRAMES || 180);
